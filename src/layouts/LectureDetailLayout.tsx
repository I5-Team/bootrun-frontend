...
export const LectureProvider = ({ courseId, data, isEnrolled, isLoading, children, recruitmentStatus }: LectureContextProps & {
...
const ProviderContainer = ({ children }: { children: React.ReactNode }) => {
  ...
  const recruitmentStatus = courseData?.recruitment_end_date ? Boolean((new Date(courseData.recruitment_end_date?.split('T')[0])) >= new Date()) : false;
  ...
  return (
        <LectureProvider 
          courseId={courseId} data={courseData} isEnrolled={isEnrolled} isLoading={isLoading} recruitmentStatus={recruitmentStatus}>
              {children}
        </LectureProvider>
  )
}
