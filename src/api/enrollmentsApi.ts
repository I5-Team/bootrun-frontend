...
import type {
  MyEnrollmentsApiParams,
  MyEnrollmentItem,
  EnrollmentResponse,
  EnrollmentDashboadItem,
  EnrollmentDetailItem,
  EnrollmentProgressResponse,
  EnrollmentProgressBodyData,
  LectureProgressResponse,
  LectureProgressBodyData,
  CourseProgressItem,
  LectureProgressItem,
...
export const updateLectureProgress = async (
  ...
  try {
    const response = await apiClient.patch(
      API_URL.ENROLLMENT.UPDATE_LECTURE_PROGRESS(lecture_id),
      bodyData
    );
    if (response) {
      console.log('[API 요청 성공]');
 ...
 * 강의실 데이터 조회 (course_id로 현재 사용자의 수강 정보 조회)
 */
export const fetchLectureRoomData = async (
  courseId: number
): Promise<EnrollmentDetailItem | null> => {
  try {
