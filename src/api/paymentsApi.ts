 ...
 * 결제 목록 조회
 */
export const fetchPayments = async (params: PaymentsParams): Promise<PaymentsItem[] | null> => {
  try {
 ...
 * 결제 상세 조회
 */
export const fetchPaymentDetail = async (payment_id: number): Promise<PaymentDetailItem | null> => {
  try {
...
export const postPaymentConfirm = async (
  payment_id: number,
  bodyData: PaymentConfirmBodyData
): Promise<PaymentConfirmResponse | null> => {
  try {
    const response = await apiClient.post(API_URL.PAYMENT.CONFIRM_PAYMENT(payment_id), bodyData);
    console.log('[API 요청 성공] 결제 확인, 반환 데이터:', response.data.data);
